...
#Method that is able to read in the folders present within the same directory as Apollo_learn
def directory(original_path, file_folder, folder_count):
    path = original_path+"\\" + file_folder
    #Changing the directory to the next folder
    os.chdir(path)
    print(f"Reading in the {folder_count + 2} folder found in directory, {file_folder}, containing: "+str(os.listdir(path)))
    print()
    #Calling on the Method that takes in the path and forms a list of .csv files within the path
    listofcsv = folder_reader(path)
    return listofcsv
...
def reportslice(report, columns):
    ...
    #Slicing the list for Preferred name
    pref_namelist = list(report.iloc[:, 4])
    #Slicing the dataframe for surnames
    surnamelist = list(report.iloc[:, 2])
    return reportlist, stkeylist, classlist, firstnamelist, pref_namelist, surnamelist
...
def subscript(csv, large_df, cohort_rows, beststudents, worststudents):
    ...
    reportlist, stkeylist, classlist, firstnamelist, pref_namelist, surnamelist = reportslice(report, columns)
                ...
                while True:
                    if userstop.isdigit():
                        ...
                        if userstop == 0:
                            ...
                            break
                    print("You have not entered 0 or 1, please re-enter a valid choice")
                    userstop = input("Do you still wish to proceed? (Enter 0 to stop the execution of the program; enter 1 to proceed): ")
    finally:
        return large_df, cohort_rows, beststudents, worststudents, reportlist, stkeylist, classlist, firstnamelist, stkeynamedict, pref_namelist, surnamelist
...
def learn(array):
    predictors = array[:, :-1]
    print("Shape of the predictor array: " + str(predictors.shape))
    n_cols = predictors.shape[1]
    target = array[:, -1]
    print("Shape of the target array: " + str(target.shape))
    #Rounding the values within the array to get a clear picture of accuracy
    target = np.round_(target, decimals = 3)
...
def training(array):
    ...
    #Rounding the values within the array to get a clear picture of accuracy
    target = np.round_(target, decimals = 3)
    #Feel free to adjust number of epochs
    my_model.fit(predictors, target, validation_split = 0.2, epochs = 10)
    my_model.save('model_file.h5')
...
def main():
    ...
    for k in os.listdir(original_path):
        if not k.endswith('.py') and not k.endswith('.h5') and not "prediction" in k and not "__pycache__" in k and not "Prediction" in k and not k.endswith('.csv'):
            filelist.append(k)
    print("Folders found within the same directory as the program: "+str(filelist))
    #Use the first folder in the directory to build the model
    first_folder = filelist.pop(0)
    path = original_path+"\\"+first_folder
    #Changing the initial directory to the first folder
    os.chdir(path)
    print("Reading in the first folder found in directory, containing: "+str(os.listdir(path)))
    print()
    #Calling on the Method that takes in the path and forms a list of .csv files within the path
    listofcsv = folder_reader(path)
    ...
    print("First folder containing the .csv files were read into the program. Now constructing Neural Network Model:")
    ...
    #Building the Model using the 'learn' method written above
    learn(imputed_array)
    #Calling a for loop to continuously train the deep learning model using the rest of the folders in the directory
    for k in range(len(filelist)):
        #Clearing the list of csv
        listofcsv = []
        listofcsv = directory(original_path, filelist[k], k)
        #Incorporate the previous methods when constructing the model
        #Initializing the large dataframe for where the year cohort's "final" scores are all stored
        large_df = pd.DataFrame()
        #Initializing an empty list containing the number of rows of the "cohort" DataFrame
        cohort_rows = []
        ...
        #Converting the dataframe into a numpy array
        array = large_df.to_numpy()
        print(f" Number {k+2} folder containing the .csv files were read into the program. Initiating training:")
        #Use KNN Imputation to fill in the missing values
        imputed_array = knn_imputation(array)
        #Printing out the numpy array and accounting for its shape
        print(imputed_array.shape)
        #Changing the directory back into the original path so that the model saves and loads in the right place
        os.chdir(original_path)
        #Run Training Method
        training(imputed_array)
